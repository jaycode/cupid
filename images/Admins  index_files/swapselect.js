...
function SwapSelect(str_pSwapId, arr_pElements, size)
{
  ...
  this.languagesValues["SwapSelectedOptionUp"] = "swap up";
  this.languagesValues["SwapSelectedOptionDown"] = "swap down";
  ...
  this.selectedStructure = __swapSel_selectedStructure;
  this.reindexStructurePositions = __swapSel_reindexStructurePositions;
  this.updateStructurePositions = __swapSel_updateStructurePositions;
 ...
 * @param   custom_structure    Array         If not empty, use this instead of this.structure
 * 
 * @return  arr_selectedItems   Array         List of the selected elements as indexes in the page selection list
 */ 
function __swapSel_getCurrentSelection(obj_pSelectElement, bol_pAll, custom_structure)
{
  ...
  for (i = 0; i < obj_pSelectElement.options.length; i++)
  {
    ...
    {
      if (typeof(custom_structure) == 'object') {
        var structure_used = custom_structure;
      }
      else {
        var structure_used = this.structure;
      }
      // Find position in original select list.
      for (x = 0; x < structure_used.length; x++)
      {
        if ((obj_pSelectElement.options[i].value == structure_used[x][0]) &&
            (obj_pSelectElement.options[i].text == structure_used[x][1]))
        {
 ...
 *
 * Change the state of options elements inside the array structure
 *
...
function __swapSel_reindexStructurePositions(arr_just_added) {
  if (typeof(arr_just_added) == 'undefined') arr_just_added = new Array();
  var active_structure = this.selectedStructure(arr_just_added);
  this.updateStructurePositions(active_structure);
}

/**
 * function  __swapSel_updateStructurePosition()
 *
 * Update this structure positions with an active_structure
 *
 * @param   arr_pEntries        Array         List of elements (indexes) that should be changed
 * @param   bol_pState          Boolean       The state to set to
 */
function __swapSel_updateStructurePositions(custom_structure) {
  //first, we reindex the positions of all (temporary) selected structure items
  for (i = 0; i < custom_structure.length; i++) {
    custom_structure[i][3] = i;
  }
  
  //then we update this object's structure with our temporary variable
  for (i = 0; i < this.structure.length; i++) {
    for (j = 0; j < custom_structure.length; j++) {
      if (custom_structure[j][0] == this.structure[i][0]) {
        this.structure[i][3] = custom_structure[j][3];
      }
    }
  }
}
...
/**
 * function  __swapSel_selectedStructure()
 *
 * Get all structure items currently in "selected" select tag.
 *
 * @param   add_just_added      Array         List of elements (indexes) just added
 */
 
function __swapSel_selectedStructure(arr_just_added) {
  var selected = new Array();
  if (typeof(arr_just_added) == 'undefined') arr_just_added = new Array();
  // get all selected structure items
  for (i = 0; i < this.structure.length; i++) {
    // if a structure item is one of just added array, put in the back, otherwise just add
    var just_added = false;
    for (j = 0; j < arr_just_added.length; j++) {
      if (i == arr_just_added[j]) just_added = true;
    }
    if (just_added == true) {
      if (this.structure[i][2] == true) {
        selected.push(this.structure[i]);
        selected[selected.length-1][3] = this.structure.length + i;
      } 
    }
    else {
      if (this.structure[i][2] == true) {
  ...
  selected = selected.sort(sort_structure);
  return selected;
}

function sort_structure(a, b) {
  return (a[3] - b[3]);
}
...
function __swapSel_rebuildSelectTags()
{
  ...
  // Add only selected elements to "selected selection" select box
  var selected_structure = this.selectedStructure();
  for (i = 0; i < selected_structure.length; i++)
  {
    // Create an option tag
    obj_optionTag = document.createElement('option');
    ...
    // Create the 'value' attribute and set the content
    obj_attributeTag = document.createAttribute("value");
    obj_attributeTag.nodeValue = selected_structure[i][0];

    // Append attribute to option tag 
    obj_optionTag.setAttributeNode(obj_attributeTag);

    // Create text node and append into option tag
    obj_text = document.createTextNode(selected_structure[i][1]);
    obj_optionTag.appendChild(obj_text);
...
function __swapSel_rebuildHiddenFields()
{
  ...
  if (obj_hiddenFiledsContainer && (obj_hiddenFiledsContainer.nodeType == 1) && (obj_hiddenFiledsContainer.nodeName.toLowerCase() == 'div'))
  {
    ...
    // Add only selected elements to hidden field container
    var selected_structure = this.selectedStructure();
    for (i = 0; i < selected_structure.length; i++)
    {
      ...
      obj_attributeTag.nodeValue = this.id.replace('][]',']['+selected_structure[i][3]+']');
      obj_optionTag.setAttributeNode(obj_attributeTag);
...
function swapSel_move(obj_pObject, direction) {
  // if direction is positive (going down), add 1 so selected items
  // would pass the item below it, or minus 1 if negative.
  if (direction != 0) {
    var obj_selectedTag = document.getElementById(obj_pObject.id + '_selected');
    var selected_structure = obj_pObject.selectedStructure();
    var selected_tags = obj_pObject.getSelection(obj_selectedTag, false, selected_structure);
    for (i = 0; i < selected_structure.length; i++) {
      for (j = 0; j < selected_tags.length; j++) {
        if (i == selected_tags[j]) {
          selected_structure[i][3] += direction;
        }
      }
    }
    if (direction > 0) {
      var next_selected_index = selected_tags[selected_tags.length - 1] + 1;
      if (typeof(selected_structure[next_selected_index]) != 'undefined') {
        selected_structure[next_selected_index][3] += -(direction + (selected_tags.length - 1));
      }
    }
    else if (direction < 1) {
      var prev_selected_index = selected_tags[0] - 1;
      if (typeof(selected_structure[prev_selected_index]) != 'undefined') {
        selected_structure[prev_selected_index][3] += -direction + (selected_tags.length - 1);
      }
    }
    
    selected_structure = selected_structure.sort(sort_structure);
    
    // Update positions
    obj_pObject.updateStructurePositions(selected_structure);
 ...
 * 
 * @param   obj_pObject         Object        The swapSel object
 * @param   bol_pState          Boolean       The state to set to
...
function swapSel_switchItems(obj_pObject, bol_pState, bol_pAll)
{
    ...
    obj_selectedTag && (obj_selectedTag.nodeType == 1) && (obj_selectedTag.nodeName.toLowerCase() == 'select')) 
  {
    ...
    if (arr_selection.length > 0) 
    {
      ...
      // Set the position of all selected elements by ascending order
      obj_pObject.reindexStructurePositions(arr_selection);
